...
public class PlayerInventory : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        InventoryFileHandler.Load(inventorySlots, offhandSlots, draggingSlot);
        if (draggingSlot.slots[0].blockType != BlockType.Nothing)
        {
            UISettings.uiPlayerInventory.StartDragging(UISettings.uiPlayerInventory.oldSlotIndex, ItemIconData.icons[draggingSlot.slots[0].blockType], draggingSlot.slots[0].amount.ToString());
            UISettings.uiPlayerInventory.uiDraggedSlotData.slots[0].gameObject.SetActive(false);
        }
    }
