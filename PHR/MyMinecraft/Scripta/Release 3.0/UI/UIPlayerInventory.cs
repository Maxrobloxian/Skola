...
public class UIPlayerInventory : MonoBehaviour
{
    ...
    internal void ToggleInventory(bool isActive)
    {
        ...
        try { if (PlayerSettings.playerInventory.draggingSlot.slots[0].blockType != BlockType.Nothing) uiDraggedSlotData.slots[0].gameObject.SetActive(true); } catch (System.Exception) { }
    }

    internal void StartDragging(int slotIndex, Sprite slotSprite, string slotText)
    {
        isDraging = true;
    ...
    internal void LoadOldSlotIndex(int slotIndex)
    {
        oldSlotIndex = slotIndex;
    }
}